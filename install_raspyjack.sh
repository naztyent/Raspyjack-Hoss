...
# ───── 5‑b ▸ WebUI headless service ──────────────────────────
WEBUI_SERVICE=/etc/systemd/system/raspyjack-webui.service
step "Installing WebUI service $WEBUI_SERVICE …"

sudo tee "$WEBUI_SERVICE" >/dev/null <<'UNIT'
[Unit]
Description=RaspyJack WebUI (headless)
...
WorkingDirectory=/root/Raspyjack
Environment=PYTHONUNBUFFERED=1
ExecStart=/bin/bash -c '/usr/bin/python3 /root/Raspyjack/device_server.py & /usr/bin/python3 -m http.server 8080 --directory /root/Raspyjack/web'
Restart=on-failure
User=root
